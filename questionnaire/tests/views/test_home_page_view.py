...
class HomePageViewTest(BaseTest):
    ...
    def test_homepage_redirects_dashboard_if_logged_in_as_global_admin(self):
        self.client.logout()
        global_admin, country = self.create_user_with_no_permissions(username="Tony")
        self.assign('can_view_users', global_admin)
        self.client.login(username=global_admin.username, password='pass')

        questionnaire = Questionnaire.objects.create(name="JRF", description="bla")
        Section.objects.create(title="section", order=1, questionnaire=questionnaire, name="section")
        Section.objects.create(title="section", order=2, questionnaire=questionnaire, name="section")
        response = self.client.get("/")
        self.assertEqual(200, response.status_code)
        templates = [template.name for template in response.templates]
