...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        for question in orm['questionnaire.question'].objects.all():
            UID = question.UID
            question.UID = UID.replace('C', '')
            question.is_core = True
            question_orders = question.orders.all()
            if question_orders.exists() and question_orders[0].order == 1:
                question.is_primary = True
            question.save()
